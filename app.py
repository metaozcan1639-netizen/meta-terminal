...
async def analyze_symbol(exchange, symbol):
    try:
        ...
        if system_state.get("ai_mode_active", True):
            ...
            reasons.append(f"🧠 YZ Analizi: Volatiliteye göre {effective_leverage}x Kaldıraç, Kasa riskine göre %{effective_risk} Risk atandı.")
        else:
        ...
        effective_risk_dollar = round((margin * effective_leverage) * (risk_dist / entry), 2)
        ...
        return {
            ...
            "margin": float(margin),
            "max_loss": float(effective_risk_dollar),
            "leverage": int(effective_leverage),
            "effective_risk_pct": float(effective_risk),
...
async def market_scanner_loop():
    ...
    add_log("Quant Motoru: YZ Otonom Yönetimi & 1.2R Minimum Hedef Devrede...")
                    ...
                    if sig and isinstance(sig, dict):
                        ...
                        if not exists:
                            ...
                            if (current_total_margin + sig['margin']) > allowed_margin:
                                add_log(f"⚠️ YZ SAVUNMA: {sig['symbol']} (${sig['margin']}) reddedildi. İzin verilen marjin boşluğu yetersiz.")
                                continue
                            
                            if sig['margin'] > system_state["free_balance"]:
                                add_log(f"⚠️ YZ SAVUNMA: {sig['symbol']} (${sig['margin']}) reddedildi. Serbest Kasa Bakiye: ${system_state['free_balance']:.2f}")
                                continue

                            system_state["active_positions"].append(sig)
                            sync_wallet_accounting()
                            mode_label = "İzole" if sig['margin_mode'] == "ISOLATED" else "Cross"
                            add_log(f"🟢 POZİSYON AÇILDI: {sig['symbol']} {sig['direction']} | {sig['score']} Puan | YZ: {sig['leverage']}x {mode_label} / Risk: %{sig.get('effective_risk_pct', system_state['risk_pct'])} | Teminat: ${sig['margin']}")
...
async def update_settings(payload: SettingsPayload):
    ...
    add_log(f"⚙️ AYARLAR GÜNCELLENDİ: Kasa: ${payload.total_balance} | Mod: {mode_str} | Manuel Risk: %{payload.risk_pct} | Manuel Kaldıraç: {payload.leverage}x | Max Poz: {pos_limit_str} | Max Marjin: %{payload.max_total_margin_pct}")
...
@app.post("/api/toggle_ai_mode")
async def toggle_ai_mode():
    system_state["ai_mode_active"] = not system_state.get("ai_mode_active", True)
    status_str = "AÇIK (Kararları Yapay Zeka Alıyor)" if system_state["ai_mode_active"] else "KAPALI (Manuel Ayarlar Geçerli)"
    add_log(f"🧠 OTONOM YZ MODU: {status_str}")
    return {"status": "success", "active": system_state["ai_mode_active"]}
    ...
    <body class="p-3 space-y-3 pb-16">
        ...
        <div class="card p-3 rounded-xl flex flex-wrap justify-between items-center gap-3 border-emerald-500/30">
            ...
            <div class="flex items-center space-x-3 text-xs text-slate-400">
                ...
                <button onclick="toggleAiMode()" id="ai-toggle-btn" class="px-2.5 py-1 rounded-lg font-bold bg-fuchsia-600 text-white hover:bg-fuchsia-500 transition shadow-[0_0_10px_rgba(192,38,211,0.5)]">🧠 YZ OTONOM: AÇIK</button>
        ...
        <div id="page-terminal" class="space-y-3">
            <div class="card p-3 rounded-xl flex flex-wrap justify-between items-center gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2 bg-slate-900/90 p-2 rounded-xl border border-slate-800 text-xs">
                    ...
                    <div>
                        <label class="text-slate-400 block text-[9px]">SABİT RİSK (%)</label>
                        <select id="input-risk" class="bg-slate-800 text-white font-bold px-1 py-0.5 rounded outline-none border border-slate-700 opacity-50 cursor-not-allowed" disabled>
                    ...
                    <div>
                        <label class="text-slate-400 block text-[9px]">SABİT KALDIRAÇ</label>
                        <select id="input-leverage" class="bg-slate-800 text-emerald-400 font-bold px-1 py-0.5 rounded outline-none border border-slate-700 opacity-50 cursor-not-allowed" disabled>
                    ...
                    <div>
                        <label class="text-slate-400 block text-[9px]">SABİT MAX POZ.</label>
                        <select id="input-max-pos" class="bg-slate-800 text-amber-400 font-bold px-1 py-0.5 rounded outline-none border border-slate-700 opacity-50 cursor-not-allowed" disabled>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-3">
                ...
                <div class="card p-4 rounded-xl lg:col-span-2 space-y-3">
                    ...
                    <div class="grid grid-cols-3 gap-2 bg-slate-900/90 p-3 rounded-xl border border-slate-800 text-center">
                        ...
                        <div><div class="text-[9px] text-slate-400 uppercase">Toplam Net PnL</div><div id="archive-prev-pnl" class="text-sm font-extrabold font-mono text-emerald-400 mt-0.5">$0.0</div></div>
                    </div>
        ...
        <script>
            ...
            async function toggleAiMode() {
                try {
                    await fetch('/api/toggle_ai_mode', { method: 'POST' });
                    updateDashboard();
                } catch(e) {}
            }
            ...
            async function updateDashboard() {
                try {
                    ...
                    if (aiBtn) {
                        if (isAiActive) {
                            aiBtn.className = "px-2.5 py-1 rounded-lg font-bold bg-fuchsia-600 text-white hover:bg-fuchsia-500 transition shadow-[0_0_10px_rgba(192,38,211,0.5)]";
                            aiBtn.innerText = "🧠 YZ OTONOM: AÇIK";
                            
                            document.getElementById('input-risk').disabled = true;
                            document.getElementById('input-leverage').disabled = true;
                            document.getElementById('input-max-pos').disabled = true;
                            
                            document.getElementById('input-risk').classList.add('opacity-50', 'cursor-not-allowed');
                            document.getElementById('input-leverage').classList.add('opacity-50', 'cursor-not-allowed');
                            document.getElementById('input-max-pos').classList.add('opacity-50', 'cursor-not-allowed');
                        } else {
                            aiBtn.className = "px-2.5 py-1 rounded-lg font-bold bg-slate-700 text-slate-300 hover:bg-slate-600 transition";
                            aiBtn.innerText = "🧠 YZ OTONOM: KAPALI";
                            
                            document.getElementById('input-risk').disabled = false;
                            document.getElementById('input-leverage').disabled = false;
                            document.getElementById('input-max-pos').disabled = false;
                            
                            document.getElementById('input-risk').classList.remove('opacity-50', 'cursor-not-allowed');
                            document.getElementById('input-leverage').classList.remove('opacity-50', 'cursor-not-allowed');
                            document.getElementById('input-max-pos').classList.remove('opacity-50', 'cursor-not-allowed');
                        }
                    }

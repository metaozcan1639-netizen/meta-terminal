...
system_state = {
    ...
    "risk_pct": 0.0,        # YAPAY ZEKA (Oto) başlangıç
    "leverage": 0,          # YAPAY ZEKA (Oto) başlangıç
    "margin_mode": "ISOLATED",
    "max_open_positions": -1, # YAPAY ZEKA (Oto) başlangıç
...
async def analyze_symbol(exchange, symbol):
    try:
        ...
        # --- YÖN BELİRLEME (Filtrelenmeden önce yönü tayin edelim ki radarda gözüksün) ---
        ...
        # --- 🚀 RADAR ÇÖZÜMÜ: Filtrelerden bağımsız tüm coinler Radara! ---
        radar_item = {
            "symbol": str(symbol),
        ...
        # --- FİLTRELEME (Radara girdikten sonra) ---
        if not direction:
            return None
        ...
        # İşlem Durdurulduysa radara yazar ama işleme girmez
        ...
        if effective_leverage == 0:
            if vol_pct > 1.5:
                effective_leverage = 5    # Çok riskli/oynak
            elif vol_pct > 0.8:
                effective_leverage = 10   # Yüksek oynaklık
            elif vol_pct > 0.4:
                effective_leverage = 20   # Orta
            else:
                effective_leverage = 50   # Düşük oynaklık (Kâr maksimizasyonu)

        if effective_risk == 0.0:
            if score >= 90:
                effective_risk = 3.0  # Mükemmel sinyal
            elif score >= 80:
                effective_risk = 2.0  # İyi sinyal
            else:
                effective_risk = 1.0  # Standart sinyal
        ...
        # --- LİKİDİTE ÖNÜNDEN KAÇIŞ VE KATI R:R DOĞRULAMASI ---
        if direction == "LONG":
                ...
                <div class="flex flex-wrap items-center gap-2 bg-slate-900/90 p-2 rounded-xl border border-slate-800 text-xs">
                    ...
                    <div>
                        ...
                        <select id="input-risk" class="bg-slate-800 text-white font-bold px-1 py-0.5 rounded outline-none border border-slate-700">
                            <option value="0.0" class="text-fuchsia-400 font-bold">Yapay Zeka (Oto)</option>
                    ...
                    <div>
                        ...
                        <select id="input-leverage" class="bg-slate-800 text-emerald-400 font-bold px-1 py-0.5 rounded outline-none border border-slate-700">
                            <option value="0" class="text-fuchsia-400 font-bold">Yapay Zeka (Oto)</option>
                    ...
                    <div>
                        ...
                        <select id="input-max-pos" class="bg-slate-800 text-amber-400 font-bold px-1 py-0.5 rounded outline-none border border-slate-700">
                            <option value="-1" class="text-fuchsia-400 font-bold">Yapay Zeka (Oto)</option>
            ...
            async function updateDashboard() {
                try {
                    ...
                    if(topBalanceEl) {
                        ...
                        topBalanceEl.className = `font-extrabold text-sm font-mono ${data.total_balance >= data.initial_balance ? 'text-emerald-400' : 'text-rose-400'}`;
                    }

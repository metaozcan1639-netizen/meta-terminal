...
def determine_auto_risk_leverage(entry, sl, score, atr=None):
    """Piyasa koşullarına göre işlem başına riski ve kaldıracı otomatik belirler.
    Manuel ayarlar yalnızca auto_risk_enabled=False olduğunda kullanılır.
    """
    balance = max(float(system_state.get("total_balance", 0)), 0.0)
    price_risk_pct = abs(entry - sl) / entry if entry else 0.0
    stop_pct = price_risk_pct * 100.0

    # Güçlü sinyale biraz daha fazla, zayıf sinyale daha az risk.
    if score >= 90:
        risk_pct = 1.25
    elif score >= 85:
        risk_pct = 1.10
    ...
    volatility = system_state.get("sentiment_data", {}).get("market_volatility", "DÜŞÜK")
    if volatility == "YÜKSEK":
        risk_pct *= 0.60
    elif volatility == "ORTA":
        risk_pct *= 0.85

    # Kasa drawdown'daysa risk otomatik küçülür.
    peak = max(float(system_state.get("peak_balance", balance)), balance, 1e-9)
    drawdown_pct = max(0.0, (peak - balance) / peak * 100.0)
    if drawdown_pct >= 8:
        risk_pct *= 0.40
    elif drawdown_pct >= 5:
        risk_pct *= 0.60
    ...
    risk_pct = round(max(0.35, min(risk_pct, 1.50)), 2)

    # Stop mesafesi büyüdükçe kaldıraç düşürülür; yüksek volatilitede ayrıca sınırlandırılır.
    if stop_pct >= 4.0:
        leverage = 5
    elif stop_pct >= 3.0:
        leverage = 7
    ...
    reason = f"Oto Risk %{risk_pct} | Stop %{stop_pct:.2f} | {volatility} volatilite | {leverage}x kaldıraç"
    system_state["auto_risk_pct"] = risk_pct
    system_state["auto_leverage"] = leverage
    system_state["auto_risk_reason"] = reason
    return risk_pct, leverage, reason


def compute_position_metrics(entry, sl, risk_pct=None, leverage=None):
    balance = max(float(system_state["total_balance"]), 0.0)
    if risk_pct is None:
        risk_pct = system_state["risk_pct"]
    if leverage is None:
    ...
    if price_risk_pct == 0 or leverage <= 0:
        return 0, 0, 0
...
async def analyze_symbol(exchange, symbol):
    try:
        ...
        if system_state.get("auto_risk_enabled", True):
            selected_risk_pct, selected_leverage, risk_reason = determine_auto_risk_leverage(entry, sl, score, atr)
        else:
            selected_risk_pct = system_state["risk_pct"]
            selected_leverage = system_state["leverage"]
            risk_reason = f"Manuel Risk %{selected_risk_pct} | {selected_leverage}x kaldıraç"

        pos_size, margin, max_loss = compute_position_metrics(entry, sl, selected_risk_pct, selected_leverage)
...
async def update_settings(payload: SettingsPayload):
    ...
    system_state["auto_risk_enabled"] = payload.auto_risk_enabled
...
@app.get("/api/state")
async def get_state():
    # Gerçekleşmiş kasa + açık pozisyonların anlık PnL'i = anlık equity.
    unrealized = sum(float(p.get("unrealized_pnl", 0.0)) for p in system_state.get("active_positions", []))
    system_state["current_equity"] = round(system_state["total_balance"] + unrealized, 2)
    system_state["used_margin"] = round(sum(float(p.get("margin", 0.0)) for p in system_state.get("active_positions", [])), 2)
    system_state["available_balance"] = round(max(system_state["total_balance"] - system_state["used_margin"], 0.0), 2)
    return system_state
            ...
            <div class="card p-3 rounded-xl flex flex-wrap justify-between items-center gap-3">
                <div class="flex flex-col space-y-1 bg-slate-900/90 p-2 rounded-xl border border-slate-800">
                    ...
                    <div class="flex items-center space-x-3 pt-0.5">
                        ...
                        <div class="border-r border-slate-800 h-6"></div>
                        <div>
                            <div class="text-[9px] text-slate-400 uppercase tracking-wider">GÜNCEL KASA</div>
                            <div id="current-balance" class="text-sm font-extrabold font-mono text-emerald-400">$1000.00</div>
                            <div id="current-equity" class="text-[8px] text-slate-500">Equity: $1000.00</div>
                        </div>
                        <div class="border-r border-slate-800 h-6"></div>
                        <div>
        ...
        <script>
                ...
                document.getElementById('active-rationale').innerHTML = `
                    ...
                    <div class="mt-2 p-2 bg-black/40 rounded border border-slate-800 text-[11px] space-y-1 font-mono">
                        ...
                        <div class="text-slate-400">Mod: <span class="text-white font-bold font-sans">${pos.leverage}x ${modeLabel} ($${pos.margin})</span></div>
                         <div class="text-amber-400 font-bold">İşlem Riski: <span class="text-white">%${pos.risk_pct || 0}</span></div>
                         <div class="text-slate-500">Risk Motoru: <span class="text-slate-300">${pos.risk_reason || 'Otomatik'}</span></div>
            ...
            async function saveSettings() {
                ...
                const res = await fetch('/api/update_settings', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({total_balance, risk_pct, leverage, auto_risk_enabled, margin_mode, max_open_positions, max_total_margin_pct})
                });
            ...
            async function updateDashboard() {
                try {
                    ...
                    const eqEl = document.getElementById('current-equity');
                    if (balEl) balEl.innerText = `$${Number(data.total_balance || 0).toFixed(2)}`;
                    ...
                    if (activeTbody) {
                        activeTbody.innerHTML = data.active_positions.map((p, idx) => `
                            <tr class="hover:bg-slate-800/80 cursor-pointer ${selectedPos && selectedPos.symbol === p.symbol ? 'bg-slate-800/60' : ''}" onclick="selectPosition(lastPositions[${idx}])">
                                ...
                                <td class="${p.direction === 'LONG' ? 'text-emerald-400' : 'text-red-400'} font-bold">${p.direction} (${p.leverage}x / %${p.risk_pct || 0})</td>
